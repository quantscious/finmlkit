...
def triple_barrier(
    ...
    n_events = len(events)  # Number of events (subset of samples)
    ...
    max_dd_ratio = np.zeros(n_events, dtype=np.float64)      # Maximum d/D ratio during the search
    ...
    for i in prange(n_events):
        ...
        rets[i] = final_ret
        max_dd_ratio[i] = max_dd

    return labels, barrier_touch_idxs, rets, max_dd_ratio
