...
def time_cues(timestamps: NDArray[np.int64]):
    ...
    :returns: tuple of arrays for the block-7 features
              (sin_td, cos_td, sin_dw, cos_dw, asia, eu, us, sess_x, top_hr)
    """
    n = timestamps.size
    # pre-allocate
    ...
    asia    = np.zeros(n, np.float64)
    eu      = np.zeros(n, np.float64)
    us      = np.zeros(n, np.float64)
    trans   = np.zeros(n, np.float64)   # optional
    top_hr  = np.zeros(n, np.float64)   # optional
